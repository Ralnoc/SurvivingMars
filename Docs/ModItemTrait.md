...
: Evaluate a match between `trait_filter` and `colonist_traits`. Returns a positive number or 0 if the Colonist matches the filter and a negative in the other case. 

Filter format is `{trait1_id = true, trait2_id = false, trait_id = nil }` To match the filter, the Colonist must have at least one of the Traits that are true, and does not have no one from Traits with false.
