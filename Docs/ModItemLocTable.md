...
This mod item allows you to specify a translation table to load to translate strings from the base game or strings from your mod.

Localization in *Surviving Mars* works by replacing texts identified by their "localization IDs" with their translated variants. Texts sometimes contain tags, delimited in <angle brackets>; nothing in tags can be modified, or the string will be corrupted. The tags in the translated string must be exactly the same as the ones in the original strings, without any deletions or additions; You are free, however, to change the order of tags, if that is required by your language.
...
*Surviving Mars* ships with fonts covering Western, Cyrillic, Korean, Japanese, Simplified and Traditional Chinese characters. To use Simplified or Traditionals, you need to add translations to the lines in the translation table describing the "font styles" used in the game; they look like this:
...
For Latin and Cyrillic-based languages, there's no need to change the font styles. For CJK lanuages, use the following font names:
...
You can adjust slightly the font sizes to compensate, or leave them as they are.
