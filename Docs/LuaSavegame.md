...
Savegames in *Surviving Mars* work by serializing the entire state of the game world. 
  ...
  * All [CObjects](LuaCObject.md) (the C++ side of the simulation) are serialized; large parts of the Lua state are also automatically serialized.
  * All Lua objects which inherit from Object are also automatically serialized, with their members.
  * Game time [Lua threads](LuaThreads.md) are assumed to be part of the simulation, and are automatically serialized.
  * Real time [Lua threads](LuaThreads.md) are assumed to be NOT part of the simulation, and are thus NOT serialized.
...
In most cases, adding new [*Object*](LuaClasses.md#toc1.4.4) descendant classes and using GlobalVars is all that's necessary to have savegames working. For the rare cases when something more is needed, the following messages can be used:
